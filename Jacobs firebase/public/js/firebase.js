...
// Firebase config (kept in gear-broken/firebase.js historically); consolidated here so
// top-level modules (`app.js`, `login.js`, etc.) can import `auth` and `db` reliably.
...
// Expose a tiny runtime diagnostic for quick debugging in the console/tests
try {
  const host = (typeof window !== 'undefined' && window.location && window.location.hostname) ? window.location.hostname : '';
  const usingEmulator = (host === 'localhost' || host === '127.0.0.1');
  console.debug('[firebase] initialized', { host, usingEmulator, authPresent: !!auth, dbPresent: !!db });
  // attach diagnostic object for manual inspection from browser console
  try { window.__FB_DIAG__ = { host, usingEmulator, authPresent: !!auth, dbPresent: !!db, authEndpoint: usingEmulator ? 'http://127.0.0.1:9199' : 'prod' }; } catch (e) {}
